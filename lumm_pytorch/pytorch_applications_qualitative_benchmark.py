...
def qualitative_benchmark_pytorch_models(log, lst_paths, np_imgs_bgr, batch_size, n_warmup, n_iters, debug=False):
  log.p('Qualitative benchmarking PyTorch {} on image tensor: {}'.format(','.join(MODELS.keys()), np_imgs_bgr.shape))
  ...
  for model_name, dct_opt in MODELS.items():
    try:
      ...
      for i,pred in enumerate(np_preds):
        ...
        path_dst = get_path_results(log, batch_size=1, fn=os.path.join(ct.PYTORCH, 
...
if __name__ == '__main__':
  ...
  qualitative_benchmark_pytorch_models(
    log=log,
    lst_paths=lst_paths,
